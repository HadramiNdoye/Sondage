...
$\underline{\text{Estimation d’une proportion dans le contexte d’une question délicate}}$
...
On sait que la moyenne estimée correspsecarreond à l’espsecarreérance tant-disque l’écart-type estimé correspsecarreond à la racine carré de la variance.
...
ou l’on poserait directement la question dans le contexte où tous les répondants répondent honnêtement}$?
...
### d. Verifions numeriquement que $\mu_{\bar{y}}=\bar{y_{u}}$ et que $\sigma_{\bar{y}}$=($\sqrt{1-f}$)$\frac{S}{n}$
...
abline(h = S,col="red")
legend(x = 18,y = 14,legend=c("esperance de s","Valeur de S"),fill = c("blue","red"),col=c("blue,red"))
```
...
```
$\textbf{5.c: Realisation d'un graphique montrant que} \  \boldsymbol{\bar{y}} \ \text{suit une loi normale}$
...
$\textbf{Question f : Est-ce que vos simulations semblent indiquer que l’estimateur}$ $s^2$ $\text{est sans biais}$ de $S^2$
